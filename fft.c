...
// 执行一维 FFT 变换，将临时缓冲区中的数据进行 FFT 计算。 FFT大小是原始数据大小的一半，即 pow2 - 1
static void do_fft(float *inout, int N_pow2, int stride) {
    int N = 2 << N_pow2;                                        // 计算傅里叶变换的数据点数
    for (int N_pow2_i = 1; N_pow2_i <= N_pow2; N_pow2_i++) {
        int N_mul2 = 2 << N_pow2_i;                             // 计算当前迭代步长的数据点数，每次迭代步长翻倍
        int N_div2 = 1 << N_pow2_i;                             // 当前迭代步长的一半
        for (int i = 0; i < N; i += N_mul2) {                   // 遍历当前迭代步长下的数据块
            for (int j = i, k = 0, l = i + N_div2, m = N >> N_pow2_i; j < l; j += 2, k += m) {
                int x0_r = (j * stride) + 0;                    // 输入数据实部索引
                int x0_i = (j * stride) + 1;                    // 输入数据虚部索引
                int x1_r = ((j + N_div2) * stride) + 0;         // 输出数据实部索引
                int x1_i = ((j + N_div2) * stride) + 1;         // 输出数据虚部索引
                // 计算蝶形运算中的乘法和加法
...
///////////////////////////////////////////////////////////////////////////////
// 为一维 FFT 控制器分配内存，并设置相关的参数，包括数据缓冲区的起始地址、长度以及计算步骤数
void fft1d_alloc(fft1d_controller_t *controller, uint8_t *buf, int len) {
    controller->d_pointer = buf;        // 一维 FFT 控制器所操作的数据缓冲区的起始地址
    controller->d_len = len;            // 一维 FFT 控制器所操作的数据缓冲区的长度
    controller->pow2 = int_clog2(len);  // 一维 FFT 所需的计算步骤数
    // 根据pow2值，分配一维 FFT 控制器所需的缓冲区空间，并将起始地址赋值给 controller 结构体中的 data 成员变量。这个缓冲区用于存储一维 FFT 的计算结果
...
// 执行一维快速傅里叶变换（FFT）操作。通过优化的数据排列方式和处理过程，可以加速 FFT 的计算过程。
// 输入数据存储在一维 FFT 控制器的数据缓冲区中，经过 FFT 变换后的结果存储回同一个缓冲区。这个函数执行完毕后，一维 FFT 的结果可以在控制器的数据缓冲区中使用
...
///////////////////////////////////////////////////////////////////////////////
// 为FFT控制器分配所需的内存，并将其存储在控制器的相应成员变量中
void fft2d_alloc(fft2d_controller_t *controller, image_t *img, rectangle_t *r) {
    controller->img = img;
    // rectangle_subimg 函数用于从图像中提取一个矩形子图像，并将结果存储在 controller->r 变量中。如果提取的子图像与原始图像没有交集，则会引发 OSError 异常并显示错误消息 "No intersection!"
    if (!rectangle_subimg(controller->img, r, &controller->r)) {
        mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("No intersection!"));
    }

    controller->w_pow2 = int_clog2(controller->r.w); // 宽度以2为底的对数
    controller->h_pow2 = int_clog2(controller->r.h); // 高度以2为底的对数
...
void fft2d_run(fft2d_controller_t *controller) {
    ...
    for (int i = 0; i < controller->r.h; i++) { 
        // Get image data into buffer.分配临时缓冲区 tmp，大小为当前行的宽度乘以 sizeof(uint8_t) 字节。用于存储图像数据
        ...
        fft1d_alloc(&fft1d_controller_i, tmp, controller->r.w); // 在每行的图像数据上分配一维 FFT 所需的缓冲区，并将临时缓冲区 tmp 赋值给一维 FFT 控制器
        fft1d_run(&fft1d_controller_i);                         // 执行一维 FFT 变换
        memcpy(controller->data + (i * (2 << controller->w_pow2)), // 将一维 FFT 变换的结果复制到主缓冲区 controller->data 中的相应位置
    ...
    // 处理二维 FFT 结果的每一列。根据主缓冲区 controller->data 的宽度来进行迭代
