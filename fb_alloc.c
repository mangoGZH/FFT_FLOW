...
// 作用:根据传递的大小和标志，分配帧缓冲内存，并进行对齐和调整，最后返回分配的内存起始地址
void *fb_alloc(uint32_t size, int hints) {
    if (!size) {
        return NULL;
    }

    size = ((size + sizeof(uint32_t) - 1) / sizeof(uint32_t)) * sizeof(uint32_t); // Round Up 向上取整
...
// 作用:分配一块指定大小的内存，并将其内容清零。如果传递的大小为零，该函数将返回空指针
